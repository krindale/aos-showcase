...
- **경매 "1등 견제 가치" 실험은 사용자 지시로 전면 롤백됨(2026-08-05l)** — 경매 코드는 원래 체계(절실함 = 내 최선−차선, Western US만 firstSeatRankBidBonus 순번 보너스) 그대로다. 실험 이력·측정치(견제 가치 전 맵 3축 게이트 통과, WU 통일 시 VP +1.8, 입찰 현금 가드의 income 무시 문제 발견 등)는 baseline 문서 2026-08-05i~l 참조 — **봇이 경매에서 상시 패스하는 문제(2인전에서 사람이 $1로 선공 독점)는 미해결 과제로 남음**. 재도전 시 그 기록부터 읽을 것.
