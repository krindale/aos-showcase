...
export default function GoodsGrowthPanel() {
  ...
  const prevGrowthRef = useRef<typeof goodsGrowthEvent | undefined>(undefined);
  useEffect(() => {
    const first = prevGrowthRef.current === undefined;
    const prev = prevGrowthRef.current;
    prevGrowthRef.current = goodsGrowthEvent ?? null;
    if (first || !goodsGrowthEvent || goodsGrowthEvent === prev) return;
    playSfx('diceLand');
    if (goodsGrowthEvent.results.length > 0) {
