...
//   승자 → 1등, 포기자는 droppedOutPlayers 역순(첫 포기자 = 꼴등, 이후 우→좌).
 ...
 * 포기자만 확정: 첫 포기자(droppedOutPlayers[0]) = 맨 우측(꼴등), 이후 우→좌로.
 * 아직 포기하지 않은 앞자리(승자 후보 포함)는 미정 = null.
 *
 * @param playerOrder 현재 플레이어 순서 (슬롯 개수 = 참가자 수의 기준)
 * @param droppedOutPlayers 포기 순서대로 쌓인 배열 (auction.droppedOutPlayers)
...
export function predictAuctionOrderSlots(
  ...
  droppedOutPlayers.forEach((pid, k) => {
    const idx = n - 1 - k;
