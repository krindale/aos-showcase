...
export abstract class MapProfile {
  ...
  /**
   * **스킵 라운드 → 엔진업 전환** 바닥값 (기본 0 = 없음 = 기존 동작).
   * 양수 ΔVP 배달이 없어 어차피 스킵할 수송 라운드에서, 스킵(0)만 이기는 소액 평가값으로
   * 엔진업을 선택하게 한다 — 배달 손실 0으로 엔진이 자란다. Scotland 2인 문제의 해법:
   * 사람이 Locomotive를 선점하면 봇 엔진이 1에 갇혀 큐브 선점에 무저항이 되는데(2026-08-05
   * 실플레이: 봇 엔진 1·income 1 vs 사람 엔진 5·income 21), 죽은 라운드 전환은 공짜다.
   * ⚠️ front-load(배달을 희생하는 선투자) 개방은 기각 — Scotland A/B에서 "엔진 올리는 동안
   * 상대가 큐브를 다 먹는" 맞대결 참패(승률 41%, VP 14.2 vs 21.4. 자기복제 VP는 +1이었는데도
   * — 대칭 측정만 믿으면 안 되는 전형). 이 바닥값은 배달을 절대 이기지 못하므로 그 함정이 없다.
   * 마지막 턴/파산 확정/상한 도달/이번 턴 기업그레이드 상태에선 적용되지 않는다.
   */
  get aiEngineSkipConversionVP(): number { return 0; }
   ...
   * 표준 절실함(내 최선 − 내 차선)만 보면 봇의 행동 가치가 평평한 턴엔 상시 양보 → 상대가
   * $1로 선공·행동 우선권을 독점한다. 2인전은 사실상 제로섬이라 상대의 (최선 − 차선)을 저지하는
   * 가치가 내 이득과 등가 — 가장 절실한 상대의 격차 ÷ 상대 수를 절실함에 더한다.
   * ⚠️ 자기복제 시뮬에선 봇끼리 격차가 작아 거의 중립(2026-08-05i: VP 15.66→15.67) — 효과는
   * 상대(사람)의 행동 격차가 클 때 발동하는 비대칭 전용. 발동하려면 위 가드 상계가 선행돼야
   * 한다(maxBid $0이면 절실함이 커도 입찰 불가).
   */
  get aiAuctionDenialValue(): boolean { return false; }

  /**
   * 경매 절실함에 **경합 수송 선순위 가치**를 더하는 배수 (기본 0 = 없음).
   * 표준 절실함(내 최선 행동 − 차선 행동)은 1등 좌석의 가치를 행동 선택권으로만 재는데,
   * 1등은 **수송 선순위**도 함께 얻는다 — 큐브가 얇은 2인 보드(Scotland)에선 서로 노리는
   * 같은 큐브를 먼저 움직이는 쪽이 가져가므로 이게 진짜 싸울 거리다. 행동 평가에선
   * First Move가 이 가치를 갖지만 절실함은 (최선−차선) 격차라 상쇄돼 사라진다(행동 8개 ÷
   ...
   * selectAction의 두 평가가 다인(3+) 전용으로 게이트돼 있다: ① Locomotive의 front-load
   * 스케줄(공짜 엔진 자산 — T1→2, T2~5→3 floor) ② Urbanization의 planUrbanization 실계획
   * (2인은 0.2 고정). 그래서 2인 Scotland 봇은 "엔진·도시화로 배달 옵션을 늘린다"는 개념
   * 없이 firstBuild만 반복했고, 사람이 Locomotive를 8턴 독점해도 경매에서 다툴 이유조차
   * 없었다 (2026-08-06 사용자 관찰: "경매는 엔진업·도시화로 옵션을 늘리는 선택인데 그런
   * 생각이 없다"). true면 두 평가가 다인과 동일하게 동작 — 봇도 그 행동들을 원하게 되어
   * 행동 격차(절실함)가 커지고 경매 경쟁이 자연 발생한다.
   */
  get aiTwoPlayerActionPlanning(): boolean { return false; }
