...
describe('St. Lucia 2 AI 전체 게임 — VP / 파산 게이트', () => {
  ...
  it('베이스라인 측정 (20 시드) — 통계 기록', () => {
    ...
    // ⚠️ timeout을 반드시 줄 것 — 20시드 전체 게임 구동은 vitest 기본 5초를 훨씬 넘는다.
    // 다른 맵 시뮬(rustBelt·germany 등)은 900_000을 주고 있는데 이 파일만 빠져 있어,
    // "측정 전용(항상 통과)" 테스트가 전체 스위트에서 늘 타임아웃으로 실패했다.
    // (부분 실행 위주라 오래 드러나지 않았다 — 2026-08-01 전체 실행에서 발견)
  }, 900_000);
