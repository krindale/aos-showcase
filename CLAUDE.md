...
│       ├── southernEnglandSimulation.test.ts # Southern England 5인(디폴트) AI 동기식 러너(7턴) + London 파랑 불변식 + 베이스라인
│       ├── scotlandSimulation.test.ts    # Scotland 2인 AI 동기식 러너(8턴) + 페리 게이트 불변식 + 베이스라인
│       └── helpers/
...
│       ├── SouthernEnglandMapProfile.ts # Southern England override (NW 큐브3/신도시 B 제거/London 파랑 대체 redirectCubePlacement)
│       └── ScotlandMapProfile.ts  # Scotland override (2인 8턴/턴오더 경매생략/경매 절반/성장 4+4/AI 계획발행 금지)
│
├── components/                 # UI 컴포넌트
...
└── utils/                      # 핵심 비즈니스 로직 유틸리티
    ...
    ├── southernEnglandMap.ts   # Southern England 맵 데이터 정의 (5~6인·디폴트 5인, flat-top 전치, 라이트/다크 도시 12·세번/트렌트/템스 강)
    ├── scotlandMap.ts          # Scotland 맵 데이터 정의 (2인 전용 8턴, flat-top 전치 9×8, 도시 6·마을 8·산+강 $5 3곳·페리 직결 2·Ayr↔Glasgow $2)
...
설명 + 플레이 버튼(`/game/<slug>/`). 튜토리얼 포함 13개 맵, Barbados만 "준비 중".
...
11개 맵 갤러리 (다인원 지원 맵은 `supportedPlayers[0]`=디폴트 인원, 인원별 턴 수는 맵 데이터
...
- **Southern England** (플레이 가능) - 잉글랜드 남부, 5~6인(디폴트 5인 7턴/6인 6턴, 사용자 결정 — 원본 룰은 4~6인) (NW 큐브 3·신도시 B 제거·London행 파랑은 주사위 1-4 NW/5-6 NE 대체)
- **Barbados** - 솔로 게임
- **Scotland** (플레이 가능) - 스코틀랜드, 2인 전용 8턴 (페리 항로 $6 도시화 게이트·Ayr↔Glasgow $2 링크·산+강 $5·턴오더 경매 생략·경매 패자 절반·성장 주사위 4+4)
- **St. Lucia** - 2인 전용
...
- **호스트 승계(팝업 방식)**: 호스트가 presence에서 사라지면 6초 유예(플랩 오탐 방지) 후, 게스트에게
  ...
  - **자발적 퇴장 ≠ 연결 끊김(`leaveSeat` intent, 2026-08-01)**: 게스트가 "방 나가기"를 누르면
    ...
    보였다. ⚠️ **대기실에서만** 비운다 — 게임 중 이탈은 끊김으로 남겨야 재접속하거나 호스트가
    봇으로 전환할 수 있다(좌석을 지우면 그 플레이어 자리가 사라진다). 차단은 하지 않는다.
    `claimSeat`·`renameSeat`과 같은 로비 전용 intent라 `INTENT_SPECS` 밖에서 처리한다.
    구버전 호스트는 이 intent를 몰라 무시 → 기존 "끊김" 표시로 동작(하위호환).
...
- **사람(비AI)의 목표 경로를 예측·등록하지 말 것 (2026-08-10, r5pm)**: 평가 코드가 사람에게 `setCurrentRoute`를 남기면(경매 견제 평가의 `ensureTurnPlan` 부수효과가 실사고) 실제 계획과 무관한 **유령 경로**가 봇들의 겹침 차단 목록을 오염시켜 top-K 전멸 → fallback 원시 커밋 → 같은 링크 병렬 부설로 이어진다. 사람을 평가해야 하면 직후 `clearCurrentRoute`+`invalidateTurnPlan`으로 정리하고, 봇 건설의 사람 겹침 회피는 **사람이 실제 완성한 링크**(`humanBuiltLinks`, selector — 이번 턴 완성분 포함) 기준으로만 한다. selector의 top-K 전멸 fallback은 상대가 잡은 sameLink를 거른다(도시 공유까지 거르면 후보 전멸 재발 — sameLink만). ⚠️ 시뮬은 전원 봇+standard라 사람 관련 경로가 **베이스라인에 안 잡힌다** — 이 계열 수정은 실게임 로그(r5pm류)로 검증할 것.
- **기각 실험도 선행조건이 바뀌면 뒤집힌다** — 막다른 길을 영구 배제하지 말 것(악화 기록은 당시 조건 한정).
- **주식 보존 법칙**: 투자(발행)를 줄이는 방향은 대체로 역효과 — income 레버만 건드린다.
- **경매 income 상계는 전 맵 기본(2026-08-08, "경매 부활")**: 입찰 현금 가드가 운영비 전액을 예비해 주식 쌓인 봇의 maxBid가 영구 $0(상시 패스 → 사람이 $1로 선공 독점, 05l 롤백의 미해결 원인)이던 것을 `aiAuctionExpensesNetOfIncome` 기본 true로 수정 — 봇 입찰 ≈0 → 8~22회/게임, 전 맵 VP −1.1 안팎은 회귀가 아니라 **경매 지출 그 자체**(새 베이스라인). **Montréal만 프로파일 격리(false)** — "경매 참여 = 과지출"이 반복 실측된 재정 최약체 맵이라 VP 12.23→6.91 붕괴로 원복. (구 기록: "1등 견제 가치" 전 맵 실험은 05l에서 롤백됐고 Scotland 훅으로 격리 재도입 — baseline 문서 2026-08-05i~08-08 참조)
- **봇 경매 성격(`AUCTION_PERSONALITIES`, 2026-08-08)**: 봇마다 경매 기질이 다르다 — standard(현 산식)/denial(1등 견제: 상대 최선−차선 가산 + 상시 $1 오프닝)/wuType(뒤 순번 +$1~2)/aggressive(절실함 ×1.25·cap $4)/conservative(×0.8·cap $2). 정의·레버는 `ai/strategy/vp.ts`, 저장은 `PlayerState.auctionPersonality`(persist·스냅샷 자동 동승), 배정은 실게임 진입점의 `randomizeBotPersonalities`(setup.ts 가중치 셔플 — denial·aggressive 3 > wuType 2 > standard 1.5 > conservative 1, 봇 ≤5명 전원 상이) **UI 비노출**. ⚠️ **시뮬·테스트는 옵션 미지정 = 전원 standard = 기존 산식 비트 항등**(rng 소비 0 — randomizeStartOrder 선례. Rust Belt·Montréal 100시드 비트 일치 실측). 중첩 가드: wuType×WU 맵 seatBonus는 **max 합성**, denial×Scotland는 맵 훅과 OR — 둘 다 중복 가산 금지(과거 중첩 붕괴 전력). 분리 측정·검증은 `personalitySeparation.test.ts` + baseline 문서 2026-08-08.
...
- **마을 가닥(스퍼) 모델**: 마을 헥스엔 타일 배치 불가 — 원→변 가닥(`buildTownSpur`)으로 연결. 타일 1개=1카운트, 가닥은 그 마을을 이번 턴 처음 변경할 때만 1카운트(가닥 개수 무관). 이동/완성/배달 판정은 가닥 있는 변으로만.
  - **⚠️ "마을 = 연결"로 뭉뚱그리지 말 것 (2026-08-10)**: 마을은 **가닥이 있는 변만** 연결이다.
    `isCityOrTown`으로 한 번에 판정하면 가닥 없는 마을 쪽 변까지 막힌 것으로 잡혀
    `isEndpointOfIncompleteSection`이 끝점을 못 찾고 **방향 전환이 조용히 거부**된다(표시
    게이트 `pickRedirectPath`는 끝점을 안 보므로 노란 칸만 뜬다 = 눌러도 무반응). 도시는
    반대로 모든 변이 암묵 연결 — 둘을 나눠서 판정한다. 도시화된 마을은 `board.cities`에
    들어가므로 도시 쪽으로 잡힌다.
  - **미소유 미완성 트랙도 가닥으로 잇는다**(룰 IV 인수 연장): `findMissingTownSpurs`가
    `owner === playerId`만 보면 주인 없는 트랙이 마을 변에서 끊겼을 때 **링크를 완성할 방법이
    아예 사라진다**(마을은 연장 후보가 못 되고, 가닥이 없으니 마을을 연결점으로 잡을 수도 없다).
    인수는 `buildTownSpur`의 `findClaimableSectionKeys`가 이미 처리한다.
  - **`buildTownSpur`의 edge 생략 ≠ edge 지정**: 생략은 "닿은 미연결 변 **전부**"를 한 번에
    짓는다(마을 클릭 = "이 마을 연결" 의도, AI도 이 경로). **방향을 고른 클릭은 반드시 edge
    지정**으로 커밋할 것 — 생략으로 커밋하면 노란 칸이 가리킨 것과 달리 같은 마을의 다른 변
    (주인 없는 트랙 쪽 포함)까지 지어져 예상 밖 비용이 나간다.
- **마을도 "노란 건설 후보"로 표시한다**: 마을은 타일을 놓는 곳이 아니라 `getBuildableNeighbors`
  에서 빠지지만, 사용자에겐 이어 지을 수 있는 칸으로 보여야 한다(마을 연결도 건설 카운트·비용을
  쓴다). 시작점의 연결 가능 변은 `hexGrid.getConnectableEdges` **한 곳**에서 계산해 연장 후보와
  가닥 후보가 같은 집합을 공유하고, 가닥 후보 판정은 `canBuildTownSpur(마을, 변)`를 그대로
  호출해 **표시=커밋**을 보장한다(미러 금지). `BoardTowns`는 후보로 뜬 마을을 다른 노란 칸과
  같은 골드 실선으로, 시작점 미선택 상태의 상시 힌트만 주황 점선으로 그린다.
- **⛔ 클릭이 조용히 실패하지 않게 할 것 (2026-08-10 제보)**: "규칙 위반인지, 차례가 아닌지,
  앱이 멈춘 건지" 구분할 수 없는 무반응이 가장 나쁘다. 후보가 아닌 헥스도 클릭을 받아
  `hintNoopClick`이 사유를 토스트로 안내한다(시작점 미선택·노란 칸 클릭·방향 클릭·물 위·
  국유화 대기·건설 제한·방향 전환 거부). 커서(pointer)만 후보에 한정해 "누를 수 있는 곳"
  표시는 유지. ⚠️ **단계 게이트가 "내 차례 아님" 안내보다 먼저**여야 한다 — 정산 단계는
  방장이 진행하는데 `currentPlayer`는 남일 수 있어, 그대로 띄우면 방장이 "남 차례네"로
  오해한다(보드 HUD를 `HUD_SUPPRESSED_PHASES`로 억제하는 것과 같은 이유).
...
- **Southern England 특수룰 (5~6인·디폴트 5인 — 정본: rules/AOSD Vol IV Rules v9 LowRes.pdf "England" + maps/southern-england-v2.pdf 인쇄문, 2026-08-04)**: 준표준 맵 — 변형 4가지뿐. ① 셋업 North West 큐브 3(`cityCubeCounts`)·그 외 2. ② **신도시 B(파랑) 게임에서 제거**(`availableNewCityTiles` — columnMapping에도 B 열 없음, 총 50칸) → London이 유일한 파랑 목적지. ③ **London행 파랑 대체**(v2 시트 "DURING SETUP & GROWTH"): 셋업·물품 성장에서 파란 큐브가 London에 놓이려 하면 주사위 1-4 → NW, 5-6 → NE — `MapProfile.redirectCubePlacement(cityId, color)` 훅(기본 항등, setup.ts 셋업 후처리 + goodsGrowthSlice 표준 성장 루프 공용). London 안의 파랑은 배달 불가능한 데드 큐브라서다(파랑 큐브는 자기 색 도시에서 이동 종료 = London에 있으면 영원히 못 나감). ④ 동점 규칙(현금→트랙→주사위)은 안내문으로만 제공(사이트는 공동 승리 표시 — Western US 선례). 원본 룰은 4~6인이나 **사용자 결정으로 5~6인**(5인 7턴/6인 6턴). 보드는 flat-top 전치 18×11(화면) → 데이터 11×18 — Rust Belt와 동일 규모·방식, 도시 12 = 디스플레이 라이트/다크 1~6 정확 대응(다크 = 검은 숫자 박스: Exeter·NW·Birmingham·Nottingham·NE·Norwich). 회귀: `utils/__tests__/southernEnglandMap.test.ts`(전사 무결성·강 끝점 인접·연결성) + `store/__tests__/southernEnglandRules.test.ts`(셋업/성장 대체) + `ai/__tests__/southernEnglandSimulation.test.ts`(20시드 게이트 + London 파랑 불변식).
- **Scotland 특수룰 (2인 전용 8턴 — 정본: rules/AOSD Exp Vol II Rules v2.pdf "Scotland"(Kevin Duffy) + maps/scotland-v2.pdf 인쇄문, 2026-08-05)**: 준표준 소형 맵(flat-top 전치 9×8, 도시 6·마을 8). ① **Turn Order 변형** — 다음 턴 경매를 통째로 생략하고 보유자가 무조건 선공(`MapProfile.turnOrderSkipsAuction`, gameStore의 issueShares→determinePlayerOrder 전환에서 `turnOrderPassAvailable` 소비·playerOrder 재배열). ② **페리 항로 2개**(Ayr↔Belfast·Ullapool↔Stornoway) — **양끝 마을이 모두 도시화된 후에만** $6 구매(`DirectLink.requiresCities` — 끝점이 board.cities로 해석돼야 buildDirectLink 통과). 도시화 전엔 마을 id를 끝점으로 갖는 잠재 링크(점선 표시만)이고, `placeNewCity`가 도시화 시 **마을 id → 신도시 타일 id로 끝점을 승계**한다. ③ **Ayr↔Glasgow $2 특수 링크** — 마을 상태에선 **표준 마을 가닥이 링크 그 자체**(기본료 $1+가닥 $1 = 룰북 £2 정확 일치, 특수 코드 없음). Ayr 도시화 시 가닥은 제거되지만 `placeNewCity`가 가닥 소유자에게 직결 링크로 **승계**(룰: "도시화돼도 제거되지 않는다"), 이후엔 도시-도시 직결 $2 구매도 가능. 이를 위해 **전 맵 공통 엔진 2건 수정**: ⓐ `canBuildTownSpur`가 인접 **도시**를 향한 가닥을 허용(도시 헥스는 hexTiles에 없어 거부되던 공백) ⓑ 사이 타일 0개인 인접 마을↔도시 가닥 링크의 수입 귀속(`hexGrid.adjacentSpurLinkOwner` — completeCubeMove 정산 + getPathLinkOwners 미러 공유. 종전엔 이 링크 수입이 무귀속으로 증발). ④ **강+산 헥스 $5** 3곳 — terrain 'mountain' + `riverEdges`(강줄기 렌더는 terrain 무관 riverEdges 우선으로 일반화) + fixedCost 5. 비용 표시는 헥스 원 숫자 대신 **건설 비용 범례의 "강+산" 조합 항목**(산 스와치+물결 아이콘 — costLegend가 mountain+riverEdges 헥스를 별도 항목으로 승격, 다른 맵은 해당 헥스가 없어 항등). 도시화 전 Ayr↔Glasgow 잠재 링크는 점선은 숨기되 **비용 원(②)은 유지 — 클릭하면 그 변 마을 가닥($2 링크)을 짓는다**(BoardCities townLink: 가닥이 지어지면 가닥 소유자 디스크로 표시, 도시화 승계 후엔 일반 직결 마커. 원까지 없애면 지을 방법이 안 보인다 — 2026-08-05 사용자 피드백). ⑤ **물품 성장 주사위 4+4**(`MapProfile.growthDiceSplit` — 앞 4개는 라이트 열(도시 1~6·신도시 A~D), 뒤 4개는 다크 열(E~H)만. 표준 맵은 null=단일 풀 항등. 주사위 개수 산출 두 곳(runAIAutoPhase·GoodsGrowthPanel)이 함께 분기). ⑥ **경매 패자 절반(올림)**(`MapProfile.auctionLoserPaysHalf` — resolveAuction에서 포기자 전원 ceil(입찰/2), 승자는 전액). ⑦ 셋업: 주머니 색깔별 −6(goodsCubeCounts 14/14/14/14/10), 도시당 2개, 디스플레이 34칸. ⑧ **AI**: 기본 긴축(`aiNoBuildIssueLastTurns=8`) + **적응형 레버리지**(`aiLeverageWhenBehind` — income이 상대 최고치보다 뒤질 때만 계획 발행 허용, 잔여 3턴+. 긴축 단독은 자기복제 과적합: 맞대결에선 레버리지 상대에게 진다(비대칭 A/B 36%→53%). 적응형 300시드 VP 15.66·income 10.27·배달 18.5·파산 0.05 — 전 지표 동시 개선, 2026-08-05h) + 완충(NetOfIncome·HopelessSkip) + **경매 상대 저지 가치**(`aiAuctionDenialValue` — 상대의 최선−차선을 입찰 상한에 합산해 상시 패스·선공 독점 차단, 2026-08-05i). engineMax 5. **분산 착공 게이트**: 독일 `aiRouteBuildGate` 재사용 — 이번 턴 슬롯·현금으로 첫 미완성 링크를 완성 못 하면 착공/계속 금지 (사용자 버그 리포트 2건: T1 분산 착공·토막 방치 + 예비금 규칙의 경로 중간 끊김. **cash가 아니라 cash−reserve로 판정** — AiRouteBuildGateContext.reserve 신설, 독일 게이트는 cash 그대로 무변. 300시드 VP 3.85→10.86·파산 0.44→**0.09**, 2026-08-05c). **회생 발행**(`aiRecapitalizeAtZeroIncome`, Scotland만 true — issueShares.ts): 계획 발행 금지 + 예비금 게이트 조합은 income 0에서 좀비 락(생존 발행 $5 = 유지비 전액 소멸, 건설 예산 영구 $0)이 되므로, **좀비 시그니처**(T3+·income≤0·주식>시작2·잔여 3턴+·목표 경로 존재)면 계획 자금까지 발행해 탈출(300시드 VP 10.86→12.54·파산 0.09→**0.03**. ⚠️ income≤0만 보면 게임 시작 상태에 발동해 붕괴 — 시그니처 필수, 2026-08-05e). 승자 분포 36/64는 좌석 편향이 아니라 **선공 불리**(진단: 시뮬 시드 진행에선 LCG 아티팩트로 전 시드 P1이 T1 선공 — 선공자 승률 35.7%가 실체. 실게임은 첫 턴 무작위라 게임 단위 공정. baseline 문서 2026-08-05g). **AI도 Ayr 링크·페리를 계획할 수 있다**(2026-08-05b): A* `canStepStationToStation`이 인접 마을↔도시 스텝 허용(가닥 하나로 완성 가능 — 인접 마을-도시 쌍은 **셋업 기준** 전 맵에서 Ayr↔Glasgow 하나뿐. 단 도시화가 마을 옆에 새 도시를 만들면 중간 생성될 수 있고 그 가닥 링크는 룰 IV상 합법 — main과의 동일 시드 A/B에서 타 맵 9/10 비트 동일, Southern England만 이 경로로 20시드 0.03 VP 미세 변화) + `tryDirectPathBuild`가 마을 쪽 가닥을 edge 지정 buildSpur로 방출 + 페리 구매 게이트에 requiresCities 예외. 단 **실효 ≈ 0**(300시드 A/B 동률, Ayr 링크 1/20게임·페리 0/20 — Stranraer↔Glasgow가 타일 하나로 직결돼 Ayr 경유가 최적이 되기 어렵고 봇이 섬 둘을 도시화하지 않음), 근거표: baseline 문서 2026-08-05b. 회귀: `utils/__tests__/scotlandMap.test.ts`(전사 무결성·연결성·섬 격리) + `store/__tests__/scotlandRules.test.ts`(경매 절반·턴오더 생략·가닥 $2/수입 귀속·페리 게이트/승계·성장 4+4) + `ai/__tests__/scotlandSimulation.test.ts`(20시드 게이트 + 페리 불변식, 최신 베이스라인 300시드 VP 15.66·파산 0.05 — baseline 문서 2026-08-05h).
...
- `src/ai/__tests__/scotlandSimulation.test.ts` - Scotland 2인 AI 동기식 전체게임 러너(8턴) + 페리 게이트 불변식 + 베이스라인
- `src/utils/__tests__/scotlandMap.test.ts` - Scotland 보드 무결성(전사·연결성·섬 격리·직결 링크) 단위 테스트
- `src/store/__tests__/scotlandRules.test.ts` - Scotland 특수룰 store 테스트(경매 절반·턴오더 경매 생략·Ayr 가닥 $2·페리 게이트/승계·성장 4+4)
