 ...
 */
export const STORE_CODE_VERSION = 12; // 소유권 회계 소유자 인식 + 가닥/복합 인수 + 복합 보조 VP
    ...
    set((state) => {
      ...
      if (state.currentPhase === 'buildTrack') {
        ...
        if (getMapProfile(state.mapId).requireCompleteLinks) {
          const r = removeIncompleteNewTracks(state.board, state.currentTurn, state.currentPlayer, getMapProfile(state.mapId).townSpurCost, getMapProfile(state.mapId).townBaseCost);
