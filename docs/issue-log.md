...
- **증상**: 우측면이 밤일 때 Serenitatis(2,10)의 화물이 맵 끝 랩 (1,10)↔(9,6)을 건너
  반대편으로 가야 하는데 운송 가이드도 안 나오고 이송도 불가.
- **원인**: 달 외곽엔 **같은 두 헥스를 잇는 랩 변이 두~세 쌍**인 곳이 많다(33번: (9,6)변5↔(1,10)변2
  + 34번: (9,6)변0↔(1,10)변3 — 삼중은 2·3·4, 15·16·17, 22·23·24, 35·36·37번). 좌표 쌍에서 변을
  역산하는 `getConnectingEdge`가 "0..5 스캔에서 처음 만나는 변"을 돌려줘, 실제 트랙이 34번 쌍에
  놓여 있으면 경로 DFS(findAllPaths/findReachableDestinations)가 33번 변으로 진입 판정 →
  상대 타일에 그 변이 없어 출구 0 = 막다른 길. 같은 이유로 AI 건설용 `getEdgeBetweenHexes`는
  **양방향 호출이 서로 다른 쌍을 골라**(A→B는 33번 변, B→A는 34번 변) 봇이 안 맞물리는 타일
  두 장을 깔 수 있었다.
- **수정**: ① `getConnectingEdge` — 후보 변이 여럿이면 실제 트랙이 놓인 변 쌍(트랙 edges/
  secondaryEdges 실존)을 선택(단일 인접은 기존과 동일 = 타 맵 항등). ② `getEdgeBetweenHexes`
  — 일반 인접 우선, 랩 인접은 최소 랩 번호 쌍으로 양방향 일관 선택.
- **검증**: `utils/__tests__/moonWrapDelivery.test.ts`(33·34 쌍 각각 양방향 배달 + 일관성) +
  `moonWrapAllPairs.test.ts`(**37쌍 전수** — 이웃 인식·진입변 정합·봇 건설 일관성, 39 항목) +
  전체 스위트 71파일 566테스트 통과. STORE_CODE_VERSION 15→16.

## 2026-08-08a — Western US 대륙횡단 보너스 오귀속 (사용자 실전 — 내 단독 연결인데 남도 +$2)

- **증상**: 내 철도만으로 동서를 연결했는데, 시작 도시에 링크만 걸친 다른 플레이어가
  보너스를 나눠 받음(+$2/+$2).
- **원인**: 보너스 판정이 **합집합 그래프 BFS가 찾은 임의 경로 하나**의 소유자 수 —
  내 단독 경로가 있어도 BFS가 남의 링크를 낀 다른(더 얕은) 경로를 먼저 찾으면 "2철도
  합작"으로 오판.
- **수정**: `computeTranscontinental` 보너스 분기 — **단독 연결(플레이어별 BFS, (1)의
  transcontinental 플래그) 우선 판정** → 있으면 그 철도만 +$4, 없을 때만 합작(각 +$2) 판정.
- **검증**: `store/helpers/__tests__/transcontinental.test.ts` 3종(단독+상대 우회 링크 존재 시
  단독 +$4 독점 / 진짜 합작 각 +$2 / awarded 조기 반환) + 전체 스위트 통과.

## 2026-08-05 — 인접 마을↔도시 "가닥만의 링크" 엔진 공백 2건 (스코틀랜드 맵 개발 중 발견, 전 맵 공통)

- **증상 ①**: 마을에서 **인접 도시를 향한** 가닥을 지을 수 없다 (`canBuildTownSpur` false).
- **원인 ①**: edge 지정 가닥 검증이 이웃 헥스를 `hexTiles`에서 찾는데 **도시 헥스는 지형
  타일이 없다** — 유효한 이웃인데 "헥스 없음"으로 거부. 기존 맵은 이 가닥을 지을 일이
  없어(트랙 낀 변만 가닥 후보) 드러나지 않았고, Scotland Ayr↔Glasgow $2 링크가 첫 실사용.
- **수정 ①**: 이웃이 도시면 허용 (`nbIsCity` — 도시는 모든 변 암묵 연결이라 가닥 = 완성 링크).
- **증상 ②**: 사이 타일 0개인 인접 마을↔도시 가닥 링크로 화물이 지나가면 **수입이 무귀속으로
  증발** (링크 소유자 판정이 타일/도시-도시 직결만 봄).
- **수정 ②**: `hexGrid.adjacentSpurLinkOwner` 신설 — 그 변 가닥의 소유자에게 귀속.
  정산(moveSlice.completeCubeMove)과 미리보기(getPathLinkOwners) 미러 동시 수정.
- **검증**: `store/__tests__/scotlandRules.test.ts`(가닥 $2 + 수입 귀속) + 전체 스위트 통과
  (기존 맵은 인접 마을-도시 가닥이 존재하지 않아 수치 무변).

## 2026-08-04c — 복합 타일의 경로를 방향 전환할 수 없음 (사용자 지적, 영국 맵 실전)

- **증상**: (6,8) 공존 타일(미소유 기본 [1,3] + player2 보조 [0,4])에서 미소유 기본 경로의
  방향을 바꿀 수 없다.
- **원인**: `canRedirectTrack`이 `trackType !== 'simple'`으로 **복합 타일을 통째로 거부**.
  룰 IV는 "복합 트랙은 다른 플레이어 소유 트랙이 유지되도록 방향 전환해야 한다" — 즉 허용이다.
- **수정**: 경로 단위 방향 전환 도입 — `trackValidation.pickRedirectPath`(조건: 경로가 미소유/
  내 소유 + 완성 링크 아님. 내 경로 우선)가 대상 경로(P/S)를 정하고, `canRedirectTrack`·
  `getRedirectableEdges`(다른 경로의 변은 새 방향 후보에서 제외)·`isEndpointOfIncompleteSection`·
  `redirectTrack`(선택 경로의 변만 교체)·`claimCompletedLinkAfterRedirect`(전환 경로에서 체인
  시작)가 전부 같은 픽을 공유한다. buildTrack의 기존 타일 교체 경로는 기본 edges만 다루므로
  복합 타일을 명시적으로 계속 거부(기존 동작 유지 — 복합 전환은 redirectTrack으로만).
- **검증**: `store/__tests__/complexPathRedirect.test.ts` 3종(미소유 P 전환 + 보조 보존 /
  타인 P·미소유 S → S 픽 + 기본 보존 / 둘 다 타인 → 거부) + store·utils·AI 전략 317개 통과.
  STORE_CODE_VERSION 13→14.

## 2026-08-04b — 복합 타일 보조 소유 디스크가 다른 경로 위에 그려짐 (사용자 지적, 영국 맵 실전)

- **증상**: 미소유 트랙 (6,8) 위에 봇(player2)이 공존을 얹자 "주인 없는 철로 위에 파란 마커가
  올라가고, 정작 파란 새 철로에는 마커가 없다" — 미소유 트랙의 소유권이 넘어간 것처럼 보임.
- **원인**: 소유 데이터는 정상(기본 경로 owner null 유지, 보조만 player2 — 로그 재구성으로 확인).
  문제는 렌더 — 보조 소유 디스크가 **헥스 중심 고정 오프셋 (x+10, y-10)**에 그려지는데, 이 타일의
  기본 경로 곡선이 정확히 그 지점을 지나가 디스크가 남의 경로 위에 얹혀 보였다. 기본 디스크도
  헥스 중심 고정이라 복합 타일(경로가 중심을 안 지남)에서 같은 계열의 오독 소지.
- **수정**: `BoardTracks.tsx` — 디스크 앵커를 **자기 경로의 중앙 침목(ties[len/2])**으로 변경
  (단순 타일 기본 디스크는 기존대로 중심 = 항등). 교차(두 직선)는 두 경로가 중앙에서 겹치므로
  앵커가 14px 이내로 포개지면 보조 디스크를 경로 1/4 지점으로 비켜 앉힌다.

## 2026-08-04 — 방향 전환으로 완성된 링크가 "미소유 완성 링크"로 동결 (사용자 지적, 영국 맵 실전)

- **증상**: southern-england 실전(game:ij8v)에서 사용자가 미소유 트랙 (2,9)를 자기 트랙 (3,9)에
  연결했는데, 다음 차례가 끝나자 **자기 트랙 (3,9)까지 미소유**가 됐다. 이후 두 타일은 어떤
  수단으로도 소유권을 되찾을 수 없는 상태(수입 0·VP 0·인수 불가)로 굳었다.
- **재구성** (:3999 로그): T3에 (2,9) 방향 전환 → **실행 취소(undo)** → (3,9) 신설(이때 (2,9)는
  전환 취소로 변이 안 맞아 연장 인수 미발동) → T4에 (2,9)를 기존 타일 교체(방향 전환 상당)로
  [2,0] 재전환 → Coventry↔Nottingham 링크가 **물리적으로 완성**됐지만 룰 "방향 전환은 연장이
  아니다"에 따라 (2,9)는 미소유 그대로 = **소유 혼합 완성 링크**.
- **원인**: 소유 혼합 링크는 룰이 상정하지 않은 상태다. 소유자 인식 완성 판정(findCompletedLinks,
  2026-07-29 회계 정합)에 잡히지 않아 releaseUnextendedTrack이 내 타일을 "미연장 미완성"으로
  해제했고, 그 결과 "미소유 완성 링크"(룰상 존재 불가능 — 인수는 미완성 구간 전용이라 복구 경로
  없음)로 영구 동결. 2026-07-29 "회계 증발"의 **방향 전환 발병 경로**.
- **수정**: `helpers/boardRules.claimCompletedLinkAfterRedirect` — 방향 전환(redirectTrack)·기존
  타일 교체(buildTrack existing 경로)가 **물리 링크를 완성**시키고 링크 소유자 집합이
  {행위자, 미소유}뿐이면 미소유 타일(경로 단위 P/S, 정부 트랙 제외)을 행위자에게 귀속한다.
  미완성이면 룰 원문대로 인수 없음(항등). 타 플레이어 경로가 섞이면 no-op(방어 — 전환 규칙상
  타인 트랙 직접 연결 자체가 금지). 인게임 로그에 "완성 링크 미소유 N타일 소유권 귀속" 표기.
- **검증**: `store/__tests__/redirectLinkClaim.test.ts` 4종(교체 경로/redirect 경로/미완성 항등/
  신설 인수 회귀) + store·utils 307개 통과 + Montréal 100시드 12.23·파산 0.73(베이스라인과
  정확 일치 = 봇 게임 항등), England 20시드 25.82(노이즈 범위). STORE_CODE_VERSION 12→13.

## 2026-08-01 — 온라인 공개 준비 중 실사용에서 나온 5건 (전부 사용자 지적)

보안 작업(S1~S4·private 채널)을 배포한 뒤 직접 플레이하며 찾은 것들. 코드 문제라기보다
**"써 보지 않으면 안 보이는" 결함**이 대부분이었다.

### ① 나가기가 들어온 화면으로 안 돌아감 — 설계 폐기

- **증상**: `/online`·`/online/quick`에서 방을 만들거나 참가한 뒤 X나 "방 나가기"를 눌러도
  그 화면이 아니라 맵 갤러리(`/maps`)나 게임 셋업에 떨어졌다. 두 번 고쳤는데 또 재발.
- **원인**: X·나가기 버튼은 5곳이지만 전부 `handleBack`/`handleLeaveRoom` 둘로 모이고, 둘 다
  **`sessionStorage['aos-back-to']`** 기록을 읽어 목적지를 정했다. 그런데 그 기록을 **지우는 지점이
  셋**(나가기 2곳 + 강퇴·승계 복귀 1곳)이라 하나만 어긋나도 조용히 폴백으로 떨어지고, 어디서
  소비됐는지 추적할 방법이 없었다. 1차 수정("읽는 쪽이 없다")은 증상만 건드린 것이었다 —
  **기록을 관리한다는 발상 자체**가 문제였다.
- **수정**: `aos-back-to`를 코드베이스에서 완전히 제거(저장 2·소비 1·정리 2 + `/maps`의
  `clearBackTo` 4개 호출과 함수). 대신 `router.back()`으로 **앱 안에서 온 직전 페이지**로 간다.
  - ⚠️ `window.history.length`로 판정하면 **다른 사이트 방문까지 세서**, 검색 결과에서 게임 URL로
    바로 들어온 사람이 X를 누르면 **우리 사이트 밖으로 나간다**. `utils/appNav.ts`가 우리가 push한
    횟수만 세도록 분리(모듈 메모리 — 저장하면 "지우는 지점"이 다시 생겨 같은 버그를 재현한다).
  - 폴백은 호출부마다 다르다: X·←·맵 선택 → `/maps`, "방 나가기" → 셋업 화면(기존 동작 보존).

### ② 게스트가 "방 나가기"로 나가도 호스트 화면엔 "연결 끊김"

- **증상**: 제 발로 나갔는데 좌석이 "끊김"으로 남아, 호스트는 그 사람이 돌아올지 기다리게 된다.
- **원인**: 게스트의 `leaveRoom`이 채널만 떠나고 **좌석엔 clientId가 그대로 남았다**. 호스트 쪽에선
  presence만 사라진 상태 = 네트워크가 끊긴 것과 완전히 같은 모습이라, 구분할 정보가 아예 없었다.
- **수정**: 채널을 떠나기 **전에** `leaveSeat` intent를 보내고 호스트가 좌석을 비운다.
  ⚠️ **대기실에서만** — 게임 중 이탈까지 좌석을 지우면 재접속도 봇 전환도 못 하게 된다.
  구버전 호스트는 이 intent를 몰라 무시 → 예전처럼 "끊김"으로 보일 뿐 깨지지 않는다(하위호환).

### ③ 연결이 끊긴 사람을 내보낼 수 없음

- **증상**: 끊긴 좌석이 자리를 차지하고 있는데 내보내기 버튼이 안 보였다.
- **원인**: 버튼 조건에 `online &&`가 걸려 있었다.
- **수정**: 빈 좌석(clientId 없음)만 제외로 완화. 끊긴 좌석엔 "봇 전환"과 "내보내기"가 둘 다 뜬다.
- **곁가지(리뷰에서 발견)**: `"봇으로 전환"`이 `clientId`만 지우고 **uid를 남기고 있었다** —
  `kickSeat` 주석이 경고하던 *"이미 나간 사람이 그 잔존 uid로 차단된다"* 의 원천이다(그래서 가드가
  있었다). 좌석 비우기를 `roomLogic.releaseSeat` 한 곳으로 합쳐 세 경로가 같은 규칙을 쓰게 했다.

### ④ 차단된 사용자가 입장을 누르면 화면이 흔들림

- **증상**: 차단 안내가 뜨면서 레이아웃이 통째로 밀렸다.
- **원인**: 두 가지가 겹쳤다. ⓐ 에러 문구가 나타나며 높이를 밀었고, ⓑ **버튼 라벨이 "입장" ↔
  "입장 중…"으로 바뀌며 폭이 변했다.** ⓑ에 고정 px(`w-[104px]`)를 줬는데도 안 먹었는데,
  flex 아이템의 `min-width: auto` 때문에 내용이 넓으면 지정 폭을 넘겨 늘어나기 때문이다.
- **수정**: 에러 자리를 버튼 아래 고정 높이로, 버튼은 **긴 라벨을 invisible로 겹쳐 두는** grid 방식
  (폰트에 의존하지 않고 항상 최대 라벨 폭을 차지한다).
- **곁가지**: 차단 확인을 채널 생성 **뒤에** 하고 `leave()`로 되돌리던 코드가
  `cannot add presence callbacks after subscribe()`를 냈다 — Supabase는 채널을 이름으로 재사용해
  구독된 채널이 남는다. 확인을 `joinRoom`의 채널 생성 **이전**으로 옮겼다.

### ⑤ 호스트가 나간 방이 DB에 계속 쌓임

- **증상**: 목록에는 안 뜨는데 `rooms` 행이 계속 누적됐다.
- **원인**: "나가기" 버튼은 `closeRoom`으로 방을 지우지만, **탭을 닫거나 새로고침하면 그 코드가
  안 돈다**(F5 자동 재입장이 그 위에 서 있어 새로고침을 방 폐쇄로 처리할 수도 없다).
  목록에서 사라지는 건 `updated_at` 2분 필터일 뿐 행은 남는다.
- **수정**: `cleanup_stale_rooms()`를 상태별 조건으로 — `waiting`·`finished` **30분**(대기실
  하트비트가 45초 주기라 30분 무활동이면 아무도 없는 게 확실하고, `finished`는 delete가 실패한
  잔재다) / `playing` 6시간(재접속 여지). 주기는 하루 2회 → **1시간**.
- **측정**: 9행·328KB(스냅샷 2개 합쳐 4.5KB) — 이 규모에선 seq scan이어도 마이크로초라 하루 24회가
  사실상 무부하다. `updated_at` 인덱스는 일부러 안 만들었다(쓰기 비용만 는다).

### 리뷰에서 뒤집은 내 진단 하나

S2 작업 중 *"`realtime.setAuth()`를 직접 안 부르면 JWT 만료로 게임 중반에 끊긴다"* 고 커밋 메시지에
썼는데, supabase-js v2.110 소스를 읽어 보니 **라이브러리가 이미 두 경로로 처리**하고 있었다
(`connect()`의 accessToken 콜백 + `_handleTokenChanged`의 `TOKEN_REFRESHED`). realtime-js에는 그 지점에
*"avoiding race conditions with SupabaseClient's immediate setAuth call"* 이라 적혀 있어, 수동 호출은
경합만 늘리는 것이었다. **setAuth 없이 재실측**(참가자 SUBSCRIBED / 비참가자 Unauthorized / join_room 후
SUBSCRIBED)해 확인하고 3곳 모두 제거했다.

## 2026-07-29c — 보조 경로 링크가 마커·국유화·성장 판정에서 빠짐 + 국유화 후보 고갈 (사용자 지적)

- **증상**: ① 교차/공존으로 완성한 링크가 **완성 링크 마커로 안 뜨고**(미완성 모양 유지),
  중국에서는 **디스크는 쓰는데 국유화 후보로 안 떠 반납할 방법이 없었다**(디스크 영구 고착).
  달에서는 보조 경로로 이은 도시가 성장 대상에서 빠졌다. ② 내 링크가 전부 당턴 건설이면
  국유화 후보가 0이 되어 상한(디스크 4)을 지킬 방법이 사라지고 5단위가 그대로 굳었다.
- **원인**: ①은 2026-07-29b에서 **VP 경로(trackValidation)만** 고치고 `hexGrid.findCompletedLinks`를
  남겨둔 탓. 그 함수는 `t.owner`만 보고 primary `edges`만 따라가서 보조 경로 링크를 통째로 놓쳤고,
  마커·국유화 대상·달 성장 판정이 전부 이 함수를 쓴다. ②는 룰의 "당턴 건설 링크 제외" 조항이
  후보를 0으로 만들 수 있는데, 그때 상한을 강제할 폴백이 없었다(안전망은 미완성 구간만 해제).
- **수정**:
  - `CompletedLink`에 `trackPaths`(좌표+경로종류 P/S) 추가, 링크 id에 경로 시그니처 포함(같은
    정거장 쌍을 잇는 기본/보조 링크 구분). `traceLinkFromTrack`이 진입 변이 속한 경로로 통과하고
    **그 경로의 소유자**를 검사.
  - `buildOwnedLinkTileIndex` 키를 (좌표+경로종류)로 → 회계 호출부에 kind 전달.
  - `eligibleNationalizationTargets`가 보조 링크도 후보에 포함(예전엔 좌표만으로 경로를 특정 못 해
    제외했다). `applyNationalization`은 보조 링크를 **secondaryOwner만** 중립화 — 타일 전체를 정부
    트랙으로 만들면 기본 경로 주인의 철도를 뺏는다.
  - 보드 소유 마커를 경로별 판정으로(한쪽만 완성된 상태가 정상).
  - `nationalizationTargets` 신설 — **후보가 하나도 없을 때만** 당턴 제외를 푸는 폴백. 표시·판정·봇이
    모두 이 함수를 쓴다(미러 금지). 평상시엔 룰대로 당턴 링크 제외(회귀 테스트 포함).
- **측정 — 전 맵 개선(9/9)**: Rust Belt 44.84→**46.44**(+1.60) · Korea +0.93 · Western US +0.85 ·
  Southern US +0.67 · China +0.61 · Montréal +0.50 · Moon +0.40 · Germany +0.25 · St.Lucia +0.10.
  파산은 유지 또는 개선(Montréal 0.72→0.67, Moon 0.13→0.12, Western US 0.22→0.21).
  **"봇은 복합을 거의 안 쓴다"던 앞선 추측이 틀렸다** — VP 경로만 고쳤을 때 항등이었던 이유는
  그때까지 `releaseUnextendedTrack`이 보조 완성 링크를 "미완성 구간"으로 보고 **미연장 시 소유권을
  해제**했기 때문이다. 봇은 교차/공존을 실제로 쓰면서 그 소유권을 매 턴 잃고 있었다.
- **검증**: 전체 회귀 474 통과 · tsc · lint 0 errors · 중국 소유 단위 상한 4 불변식 유지.

## 2026-07-29b — 교차/공존의 보조 경로로 완성한 링크가 VP 0 (사용자 지시로 수정)

- **증상**: 복합 타일(교차·공존)의 **보조 경로**로 도시↔도시를 이어 링크를 완성해도 승점이
  붙지 않는다. 수입은 정상 지급돼(`getPathLinkOwners`가 진입 변 기준으로 귀속) 발견이 어렵다.
- **원인**: `isCompletedLink`/`findAllCompletedLinks`(trackValidation)가 **primary `edges`만**
  추적했다. 시작점 판정도 `t.owner === playerId`만 봐서 `secondaryOwner`를 놓쳤고, `allOwned`도
  경로 종류를 구분하지 않았다. 게다가 `calculateTrackScore`는 좌표 Set으로 집계해, 설령 잡히더라도
  같은 헥스의 기본·보조가 **하나로 합쳐져** 점수가 누락될 구조였다.
- **수정**: ① `isCompletedLink`가 진입 변이 속한 경로(P/S)로만 통과 — 이동 경로 탐색
  (`hexGrid.checkConnectionToCity`)이 이미 쓰던 규칙과 통일. 순환 감지도 **(헥스+경로종류)**
  단위로 바꿔 복합 타일의 두 트랙을 한 경로가 각각 지나는 합법 케이스를 허용. ② 시작점에
  `secondaryOwner` 포함, `allOwned`를 경로 종류별 소유자로 검사. ③ `calculateTrackScore`
  집계 단위를 **(헥스+경로종류)** — 룰북 "완성된 철도 링크의 각 트랙 구간당 +1점"에서 복합
  타일의 두 트랙은 독립 구간이므로 각각 1점이다.
- **측정**: 계획 단계에선 "달 성장 판정·AI 평가가 바뀌는 의미 변경"으로 보고 보류했으나,
  **전 맵 100시드 실측 결과 사실상 항등**(정확 항등 6 · ±0.03 이내 2, 파산 전 맵 동일).
  봇이 교차/공존을 거의 쓰지 않아 실효는 **사람 플레이의 누락 점수 복구**에 한정된다.
  보류 판단이 과했던 셈. 표: [docs/ai-auction-baseline-100seed.md](ai-auction-baseline-100seed.md).
- **남은 범위**: VP 경로만 고쳤다. `hexGrid.findCompletedLinks`(디스크 회계·완성 링크 마커·
  달 성장 도시)는 여전히 primary만 추적한다 — 디스크는 `countUnfinishedSections`가 보조 구간을
  세어 결과적으로 일관되지만, **보드의 소유 마커는 보조 경로로 완성한 링크에서도 "미완성"
  모양으로 남는다**(시각 불일치, 미수정).
...
- **증상**: 경매에서 **포기할 때마다** 채워지는 "새로운 순서" 색 원의 팝 애니메이션이 사각형
  경계에서 잘려 정상적으로 보이지 않는다.
- **원인**: 그 영역은 `width: 0 → auto`로 옆에서 밀고 들어오는 연출이라 `overflow-hidden`이
  필요한데, **펼쳐진 뒤에도 계속 켜져 있었다**(TurnTrack.tsx). 원의 팝은 공용
  `POP_SPRING`(stiffness 480·damping 16 → 감쇠비 ≈0.37 **underdamped**)이라 `scale`이 1을 넘어
  튀는데(≈1.2), 그 초과분이 컨테이너에 잘렸다. 상위 헤더에는 overflow가 없어 원인은 이 한 곳.
- **수정**: 펼침 애니메이션이 끝나면(`onAnimationComplete`) `overflow-hidden`을 해제.
  닫힐 때는 `showNewOrder` 변화를 보는 effect가 먼저 돌아 다시 잘라주므로 exit의 `width→0`도
  깔끔하다. 스프링 상수는 다른 팝 연출과 공유하므로 건드리지 않았다.

## 2026-07-28 — 운송 가이드가 차례·단계 전환 후에도 남음 (사용자 발견)

- **증상**: 선택된 운송 가이드(목적지 골드 링·최적 경로 점선·선택 큐브 강조)가 차례가 넘어가도,
  물품 이동 단계가 아닐 때도 계속 보인다.
- **원인 — 상태·렌더 양쪽 모두 게이트가 없었다**: ① `nextPhase`의 단계 전환 분기들은 **건설
  UI만** 손질하고(`buildMode`·`sourceHex`·`previewTrack` …) 화물 쪽(`selectedCube`·
  `reachableDestinations`·`movePath`·`routeOptions`·`routeChoice`)은 **어디서도 지우지 않았다**
  (`undoLastAction`만 일부 정리). ② GameBoard가 이 값들을 `currentPhase` 조건 없이 넘겼다 —
  개인 설정(`moveGuideOn`)만 보고 단계는 안 봤다.
- **수정**: `nextPhase` **초입**에서 일괄 정리(분기마다 넣으면 또 샌다) + GameBoard에 단계
  게이트(`moveGoodsPhase`/`moveGuideVisible`)로 이중 방어. `movingCube`는 제외 — 진행 중
  애니메이션은 `completeCubeMove`가 끝내며, 그 액션은 ui를 전부 비운 뒤 `nextPhase`를 부른다.
- **검증**: 회귀 테스트 추가 후 수정을 되돌려 실패 확인(`expected { cityId: 'P', cubeIndex: 0 }
  to be null`). STORE_CODE_VERSION 9 → 10.

## 2026-07-28 — 직결 링크(GZ↔HK 페리) 화물 애니메이션 끊김 (사용자 발견)

- **증상**: 남부 중국에서 광저우(파란 2번)→홍콩 $8 직결 링크로 화물을 옮기면 **점선 경로는
  그려지는데 큐브가 움직이지 않는다**.
- **원인**: GZ↔HK는 **변으로 인접하지 않은** 도시 쌍(원본 시트대로 면 앵커 직선 렌더)이라
  `getConnectingEdge`가 board를 줘도 null이다. `getAnimationPoints`는 변 중점을 못 구하면 그
  구간 포인트를 **하나도 만들지 않아**, `[GZ, HK]` 경로에서 출발 도시 중심 1개만 남았다 →
  호출부(BoardOverlays)의 `times: i/(length-1)`가 **0/0 = NaN** → Framer Motion이 통째로 깨진다.
  인접 직결 쌍(GZ↔SZ·SZ↔HK)은 공유 변이 있어 멀쩡했던 것도 이걸로 설명된다.
- **수정**: `midpointTowardHex` 폴백 — 변이 없으면 두 중심의 중점을 경계 통과점으로
  (직결 링크는 렌더도 직선이라 시각적으로 일치). `getAnimationPoints`·`getMovementPathSVG` 공유.
  ⚠️ 달 랩 어라운드는 board 기준 **인접**이라 기존 subpath 분리 로직 그대로.
  곁다리: 중간 헥스 처리가 `track`/`isTown`만 다뤄 **경로 중간의 도시**는 진입점만 찍고
  건너뛰었다(큐브가 도시 폭만큼 순간이동) → 정거장이면 마을·도시 구분 없이 중심 경유.
- **검증**: 신규 테스트 5종(`directLinkAnimation`). 수정을 되돌려 `expected 1 to be >= 2` 실패 확인.

## 2026-07-28 — 화물 가이드: 내 수입 동률인 타인 경유 경로 노출 (사용자 지시로 정책 되돌림)

- **증상/지시**: 「내 철도 2링크」와 「내 2 + 남 1」처럼 내 수입이 같은데 남의 철도를 끼는 경로가
  경로 선택 목록에 떴다. 상대에게 수입만 헌납하는 선택지라 **목록에도 두지 말 것**.
- **수정**: `findRouteOptions`(`< threshold` → `<=`)·`gateMixedByCubeBest`(`>=` → `>`).
  본인 철도로 **도달 불가한** 목적지의 유일한 경로는 그대로 노출(불변).
...
- **곁다리 버그**: 목적지 **간** 골드 점선 비교(uiSlice)가 `ownLinks → totalLinks 최대`만 봐서
  내 수입 동률이면 **총 링크가 긴 쪽**이 이겼다 — 본인 철도로 갈 수 있는데도 남의 철도를 낀 더 먼
  목적지를 추천. 같은 파일의 트랙 큐브 경로는 원래 `oppIncome`을 보고 있어 셋의 기준이 어긋나
  있었다 → 도시·마을·트랙 큐브 모두 `내 수입 최대 → 빌린 링크 최소 → 총 링크 최대`로 통일.
- **측정(9개 맵 100시드)**: 회귀 0 · 개선 3(Moon 21.01→21.38 · Germany 35.69→35.95 파산
  0.19→0.16 · Korea +0.02) · 정확 항등 5. 상세: [baseline 2026-07-28](ai-auction-baseline-100seed.md).

## 2026-07-28 — 국유화 대기 중 봇 전환 시 AI 무한루프 (최종 리뷰 F2, 온라인 한정)

- **증상**: 남부 중국에서 국유화 대기(`nationalizationPending`)가 선 채로 그 플레이어가
  봇으로 전환되면 — 온라인 게스트 이탈(`convertSeatToBot`) 또는 호스트 승계 시 옛 호스트
  봇 전환(`promoteToHost`) — 게임이 진행되지 않으면서 CPU를 계속 먹는다.
- **원인**: 봇에겐 국유화 선택 UI가 없고, 봇 자동 해소는 **건설 직후**(`afterBuildDiscCheck`)
  에만 돌아 전환 시점엔 이미 지나간 뒤다. 그 상태에서 `nextPhase`는 buildTrack 단계의 대기를
  보고 `return state`로 **보류**하지만 함수 끝에서는 **항상** `scheduleAICheck`를 부른다 →
  봇 결정(건설 불가·skip) → nextPhase 보류 → 재스케줄 → … 순환이 끝나지 않는다.
- **수정**: 봇 해소 로직을 `resolveBotNationalization(set, get)`으로 추출·export하고
  `executeAITurn`의 buildTrack case 진입 직후에도 호출(해소했으면 락 해제 후 재결정).
  `afterBuildDiscCheck`도 같은 함수를 쓴다 — 판정이 한 곳(미러 금지).
- **테스트**: `southernChinaNationalization.test.ts`에 2건(봇 전환 후 자동 해소 + 상한 복원 /
  사람 차례엔 미개입). 무한루프 자체는 safeTimeout 실행 경로라 재현 대신 해소 계약을 박제.
- **회귀 없음**: 리팩터 후 100시드 게이트가 VP 17.21·파산 0.35로 이전과 정확히 일치(항등).

## 2026-07-28 — 취소 버튼이 패널을 세로로 늘려 화면이 밀림 (사용자 피드백)

- **증상**: 취소 가능 상태가 될 때마다 PhasePanel 높이가 늘어 화면 전체가 움직이는 느낌.
- **원인**: 행동 선택·트랙 건설·물품 이동 단계에서 실행 취소/선택 취소 버튼이 진행 버튼과
  **별도 행**(w-full)으로 쌓였다. 물품 이동은 위아래로 2행이 늘었다.
- **수정**: 취소 계열을 진행 버튼과 같은 `flex` 행 우측에 배치(진행 `w-full`→`flex-1`,
  취소 `flex-shrink-0`) — 취소가 나타나도 행 높이 불변, 진행 버튼 폭만 준다. 선택 취소는
  폭 절약을 위해 X 아이콘만(title·aria-label 유지). 정부 링크 단계는 좌측이던 취소를
  우측으로 통일. 주식 발행은 이미 같은 형태라 무변경.
